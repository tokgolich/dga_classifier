...
fi = open('data/training_w_tld.txt','r')
fw = open('data/features.txt','w')
...
'''
header = 'ip\tclass\tentropy\tlen\tnorm_entropy\tvowel_ratio\tdigit_ratio\trepeat_letter\tconsec_digit\tconsec_consonant\tgib_value\thmm_log\tuni_rank\tbi_rank\ttri_rank\tuni_std\tbi_std\ttri_std\n'
fw.write('%s'%(header))
...
for f in fi:
    ...
    try:
        fw.write('%s\t%s\t%.3f\t%.1f\t%.3f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n'
                %(domain,cla,entropy,f_len,entropy/f_len,vowel_ratio,
